...
const onBoardings = [
	{
		...
		description: "Our expert online interior designers will transform any room in your home in your style and budget",
		img: onboarding1,
	},
	{
		title: "Experience Designs in 3D",
		description: "On our interactive 3D desktop App, see your actual room designed in 3D with handpicked products",
		img: onboarding2,
	},
	{
		title: "Shop From Your Designs",
		description: "Shop furniture and decor from your design all at once. Or, bring your room to life",
		img: onboarding3,
	},
];
...
const WalkThrough = ({ navigation }) => {
	...
	React.useEffect(() => {
		scrollX.addListener(({ value }) => {
			Math.floor(value) > SIZES.width ? setCompleted(true) : setCompleted(false);
		});
		...
		return (
			<Block style={styles.dotsContainer}>
				{onBoardings.map((item, index) => {
					...
					return <Animated.View key={`dot-${index}`} opacity={opacity} style={[styles.dot]} />;
				})}
			</Block>
		);
	};
	...
	return (
		<Block>
			...
			<Block style={styles.skipHolder}>
				<Button color={COLORS.semiTransparent} size="xs" onPress={() => navigation.navigate("Home")}>
					<Text>SKIP</Text>
				</Button>
			</Block>
